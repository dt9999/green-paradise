...
(async () => {
  ...
  try {
    for (const [name, viewport] of [['desktop', { width: 1365, height: 850 }], ['mobile', { width: 844, height: 390 }], ['portrait', { width: 390, height: 844 }]]) {
      ...
      await page.goto(url);
      assert.match(await page.locator('#version').textContent(), /1.1.0/);
      ...
      assert.equal(await page.locator('#researchNotes details').count(), 1);
      await page.locator('#researchNotes summary').tap();
      assert.ok((await page.locator('#researchNotes p').textContent()).length > 20);
      ...
      assert.equal(await page.evaluate(() => JSON.parse(localStorage.getItem('greenParadiseSaveV1')).records.length), 1);
      if (name === 'mobile') {
        await page.evaluate(() => { const create = Paradise.createGame; Paradise.createGame = (...args) => (window.testGame = create(...args)); });

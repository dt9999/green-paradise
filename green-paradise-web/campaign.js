(function (root) {
  ...
  const STORY_AREAS = [
    ["目をさます大地", "風の音だけが、からっぽの世界を歩いていた。", "ぼくの足あとに、芽が出た……。もっと遠くまで歩いてみよう。", "小さな緑が、はじめて荒野をつないだ。", "この先の街にも、緑を届けたい。"],
    ["砂の下の暮らし", "砂に埋もれた道には、人が暮らしていた跡がある。", "ここにも、誰かの帰る場所があったんだ。", "砂のすきまから、細い根が水を探しはじめる。", "緑を奪っているものを、追いかけよう。"],
    ["地下からの声", "水晶の光の奥で、古い配線がかすかにうなっていた。", "自然の洞窟なのに、こんなところまで機械が……？", "静かになった洞窟に、声のような残響がひとつ残った。", "今の音……ただの機械の音じゃなかった。"],
    ["止まった工場", "動かない工場。その奥へ、吸収された光だけが流れていく。", "誰が、何のために、この場所を作ったんだろう。", "割れた外殻のそばに、人が使う小さな道具が落ちていた。", "この敵たち、人の暮らしとつながっているのかな。"],
    ["湿地に残る名前", "水の上に、病室の窓が並んでいた。呼びかけに返事はない。", "助けを待っていた人たちは、どこへ行ったんだろう。", "消えていく影が、人のように窓へ手を伸ばした。", "もしかして……きみたちは、人間だったの？"],
    ["雪に閉じた研究", "雪の下で眠る診療所。扉には、研究主任の印が残る。", "ここで何が起きたのか、確かめなくちゃ。", "白い地面に、緑が戻る。けれど、疑問は消えなかった。", "科学者が、この体に変えたのかな……。なぜ？"],
    ["流れの向こうへ", "谷を走る導管は、遠くの研究施設へ集まっていた。", "みんなの力を、あそこへ運んでいるんだ。", "導管の振動が弱まる。向こう岸の施設が、静かに光った。", "答えは、研究所にある。行こう。"],
    ["夜の研究所", "閉ざされた研究棟で、まだ動く警報が点滅している。", "誰もいないのに、研究は止まっていない……？", "壊れた画面に、ひとつの計画名が浮かび上がった。", "ゼロパラダイス……。これが、緑を奪う力の中心？"],
    ["灰色の地球", "観測所の窓の向こう。地平線まで、灰色の大地が続く。", "こんなに遠くの緑まで、奪われていたんだ。", "封鎖された中枢へ続く道が、ゆっくりと開いた。", "奪われた光は、まだ消えていない。取り戻せる。"],
    ["ゼロの庭", "大地の奥から響く鼓動。すべての導管が、そこへ向かう。", "もうすぐだ。世界の緑を、返してもらおう。", "吸収炉を包んでいた闇に、ひびが走った。", "聞こえる。閉じこめられていた、世界の息づかいが。"],
  ];
  function story(stage, trigger) {
    ...
    if (stage.id === 50) {
      ...
      lines = [line("ナレーション", `${stage.boss.name}が倒れ、あたりに静けさが戻った。${a[3]}`), line("グリーン", a[4])];
    } else if (trigger === "goal") {
      lines = [line("ナレーション", `${stage.name}の大樹に、緑の光がともった。`), line("グリーン", ["この木が、次の芽を守ってくれる。", "少しずつでも、ちゃんと戻ってきている。", "道はつながった。次の場所を探そう。", "この景色を、誰かにも見せたいな。", "まだ旅はつづく。もう少し、先へ行こう。"][stage.local])];
    } else {
      const observations = [a[1], `${stage.sections[0].name}へ続く道に、古い足あとが残っている。`, "風がやむと、遠くの施設から低い音が聞こえた。", "枝のない木々の向こうに、次の道が見える。", `${stage.boss?.name || "大きな影"}が、この先で道をふさいでいる。`];
      lines = [line("ナレーション", observations[stage.local]), line("グリーン", stage.local === 0 ? a[2] : ["", "ここを通った人は、何を見たのかな。", "急がずに、周りもよく見て進もう。", "残された記録も、見逃さないようにしよう。", "この先へ進むために……負けられない！"][stage.local])];
    }
    return { id: `${PART.id}-${stage.id}-${trigger}`, title: a[0], stage: stage.id, trigger, lines };
  }
